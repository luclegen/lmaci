...
export class ProductsComponent implements OnInit {
  ...
  onSearch(form: NgForm) {
    this.authService.getInfo().subscribe(res => { if (res['user'].role == 'root' || res['user'].role === 'admin') this.adminService.searchProducts(form.value).subscribe(res => this.products = res['products'], err => alert(err.error.msg)); }, err => { if (err.status == 440 && confirm('Login again?\nYour session has expired and must log in again.')) window.open('/login'); });
  }
